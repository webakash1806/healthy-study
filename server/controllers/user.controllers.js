...
const updateProfile = async (req, res, next) => {
    try {
        ...
        const { fullName, address, phoneNumber, alternateNumber } = req.body
        const { id } = req.params
        ...
        if (alternateNumber) {
            if (user.phoneNumber) {
                ...
                user.alternateNumber = await alternateNumber
            } else {
                return next(new AppError('Please enter phone number first', 400))
            }
        }
...
const addDeliveryData = async (req, res, next) => {
    try {
        const { id } = req.params
        const { address, phoneNumber, alternateNumber } = req.body
...
const addNumber = async (req, res, next) => {
    try {
        const { id } = req.user
        const { phoneNumber, alternateNumber } = req.body
